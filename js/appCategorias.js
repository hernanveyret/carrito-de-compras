...
let categoria = localStorage.getItem('miCategoria');   // tomo la categoria que se elijio ej colegial.
let seleccionPor = localStorage.getItem('seleccionPor'); // seleccionado por ej: categoria, estilo, marca, material
let estilos = localStorage.getItem('miEstilo'); // seleccion por estilo ej. zapatilla, ojota.
let material = localStorage.getItem('miMaterial'); // seleccion por material ej : cuero, lona.
    ...
    seleccionPorMaterial()
        ...
        function seleccionPorCategoria() { 
            if ( categoria === 'niños') {
                ...
                for ( let indice in articulos ) {
                  if (articulos[indice].subEstilo === 'colegial') {
                    mostrarCards(indice);
                  }
                }
              }
