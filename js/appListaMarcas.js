...
fetch('js/base.json').then(response =>{ 
  return response.json();   
        ...
        let html = '';
            html += `<h3 class="hTitulo">MARCAS</h3>`;      
            ...
            // Menu de estilos
        let htmlE = '';
            htmlE += `<h3 class="hTitulo">ESTILOS</h3>`;
            ...
            // Menu de materiales
        let htmlM = '';
            htmlM += `<div class="degradado"><h3 class="hTitulo">MATERIALES</h3>`;
      ...
      let contenedor = document.querySelector('.contenedor');
        ...
        const btnlista = e.target.classList.contains('btn-marcas');
            if ( btnlista == true ){ 
                let marca = objetoSelect.querySelector('.btn-marcas').dataset.id
                   localStorage.setItem('tipoDeEleccion','marca');
                   localStorage.setItem('tuEleccion',marca);
...
const selectorCategorias = document.querySelector('.cardsCategorias');
      ...
      const btnCategorias = e.target.classList.contains('imgGenero');
            if (btnCategorias) {
              ...
              localStorage.setItem('tipoDeEleccion','categoria');
              localStorage.setItem('tuEleccion',miCategoria);
              location.href="carritoPorCategoria.html";
            }
    })
...
function estiloMenu() {
  ...
  localStorage.setItem('tipoDeEleccion','estilo');
  localStorage.setItem('tuEleccion', miEstiloSeleccion);
  location.href="carritoPorCategoria.html";
}
// Selector menu por material
function materialesMenu() {
  let miMaterialSeleccion = document.getElementById('menuPorMateriales').value;
  localStorage.setItem('tipoDeEleccion','material');
  localStorage.setItem('tuEleccion', miMaterialSeleccion);
  location.href="carritoPorCategoria.html";
